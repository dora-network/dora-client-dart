...
class PLResponseEnvelope {
  ...
  static PLResponseEnvelope? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'metadata'), 'Required key "PLResponseEnvelope[metadata]" is missing from JSON.');
        assert(json[r'metadata'] != null, 'Required key "PLResponseEnvelope[metadata]" has a null value in JSON.');
        return true;
      }());

      return PLResponseEnvelope(
        data: PLAccount.listFromJson(json[r'data']),
