...
class ResponseEnvelopeOfListAssets {
  ...
  static ResponseEnvelopeOfListAssets? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'metadata'), 'Required key "ResponseEnvelopeOfListAssets[metadata]" is missing from JSON.');
        assert(json[r'metadata'] != null, 'Required key "ResponseEnvelopeOfListAssets[metadata]" has a null value in JSON.');
        return true;
      }());
