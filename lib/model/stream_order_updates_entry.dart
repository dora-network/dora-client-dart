...
class StreamOrderUpdatesEntry {
  ...
  static StreamOrderUpdatesEntry? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'Val'), 'Required key "StreamOrderUpdatesEntry[Val]" is missing from JSON.');
        assert(json[r'Val'] != null, 'Required key "StreamOrderUpdatesEntry[Val]" has a null value in JSON.');
        assert(json.containsKey(r'Time'), 'Required key "StreamOrderUpdatesEntry[Time]" is missing from JSON.');
        assert(json[r'Time'] != null, 'Required key "StreamOrderUpdatesEntry[Time]" has a null value in JSON.');
        return true;
      }());
