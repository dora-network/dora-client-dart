...
class UserValueResponseEnvelope {
  ...
  static UserValueResponseEnvelope? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'metadata'), 'Required key "UserValueResponseEnvelope[metadata]" is missing from JSON.');
        assert(json[r'metadata'] != null, 'Required key "UserValueResponseEnvelope[metadata]" has a null value in JSON.');
        return true;
      }());
