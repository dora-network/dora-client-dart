//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:dora_client/api.dart';
import 'package:test/test.dart';

// tests for OrderBook
void main() {
  // final instance = OrderBook();

  group('test OrderBook', () {
    // String orderBookId
    test('to test the property `orderBookId`', () async {
      // TODO
    });

    // int orderBookSeq
    test('to test the property `orderBookSeq`', () async {
      // TODO
    });

    // num baseQuantity
    test('to test the property `baseQuantity`', () async {
      // TODO
    });

    // String baseAssetId
    test('to test the property `baseAssetId`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // String displayName
    test('to test the property `displayName`', () async {
      // TODO
    });

    // int baseAssetFractionalizedUnits
    test('to test the property `baseAssetFractionalizedUnits`', () async {
      // TODO
    });

    // int quoteAssetFractionalizedUnits
    test('to test the property `quoteAssetFractionalizedUnits`', () async {
      // TODO
    });

    // num feeFactor
    test('to test the property `feeFactor`', () async {
      // TODO
    });

    // num initialAssetsRatio
    test('to test the property `initialAssetsRatio`', () async {
      // TODO
    });

    // DateTime maturityAt
    test('to test the property `maturityAt`', () async {
      // TODO
    });

    // num quoteQuantity
    test('to test the property `quoteQuantity`', () async {
      // TODO
    });

    // String quoteAssetId
    test('to test the property `quoteAssetId`', () async {
      // TODO
    });

    // num sharesQuantity
    test('to test the property `sharesQuantity`', () async {
      // TODO
    });

    // OrderBookStatus status
    test('to test the property `status`', () async {
      // TODO
    });

    // num tickSize
    test('to test the property `tickSize`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // DateTime haltedAt
    test('to test the property `haltedAt`', () async {
      // TODO
    });

    // DateTime terminatedAt
    test('to test the property `terminatedAt`', () async {
      // TODO
    });

    // DateTime poolUpdatedAt
    test('to test the property `poolUpdatedAt`', () async {
      // TODO
    });

    // String sharesAssetId
    test('to test the property `sharesAssetId`', () async {
      // TODO
    });


  });

}
